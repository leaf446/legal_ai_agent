...
- **LLM 호출 시 신뢰 경계 이탈**: 증거의 *저장*은 AWS 계정 내부로 격리했지만, STT·Vision·초안 생성 등
  LLM API 호출 시에는 증거 내용이 외부(OpenAI/Google) 서버를 거친다. 민감정보의 국외 이전과
  변호사 비밀유지의무 관점에서 이 지점이 현재 아키텍처의 가장 큰 한계라고 판단한다.
  → **개선 설계**: 기존 Gemini/OpenAI 폴백 구조에 온프레미스 LLM(Ollama) 프로바이더를 추가해
  민감 사건은 로컬 모델로 처리하는 하이브리드 모드. 단, 로컬 LLM은 Lambda에 탑재할 수 없어
  배포 형태가 온프레미스 패키징으로 확장되는 결정이며, 임베딩 교체는 사건별 벡터 재인덱싱이
  필요해 후순위로 구분했다.
- **프론트엔드 테스트 커버리지**: 테스트 2,000여 개가 통과하지만 커버리지는 35% 수준.
  핵심 플로우(증거 업로드 → 분석 → 초안) 중심으로 커버리지 확대 여지가 있다.
- **접근성(WCAG)**: 개발 후반 UI 개편 과정에서 키보드 내비게이션·스크린 리더 지원이 축소됨.
  공공기관·대형 로펌 납품 시 필수 요건이므로 재도입이 필요하다.
